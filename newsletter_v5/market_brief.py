...
before it ships:
  ...
  2. Verify    — deterministic checks only: em dash / banned-word autofix,
                 length trim, and the number-grounding gate (every figure in
                 the brief must appear in a source summary). The earlier
                 LLM claim-audit pass was removed: its findings were
                 advisory-only (logged, never acted on) while costing 1-3
                 full-source-text Haiku calls per run.
  3. Fail-safe — if the brief still fails after regeneration, salvage the
                 grounded sentences or return None and ship without it. A
                 missing brief beats a confident-but-wrong one.
...
class MarketBriefGenerator:
    ...
    async def _call(self, system: str, user: str, tool: dict, tool_name: str) -> Optional[dict]:
        ...
        THIS CALL once on the fallback model; never switches subsequent
        calls (a permanent Sonnet switch triples the price silently)."""
        for model in (CLAUDE_MODEL_PRIMARY, CLAUDE_MODEL_FALLBACK):
            for attempt in range(CLAUDE_MAX_RETRIES):
                try:
                    ...
                    self.metrics.add_usage("brief", model, response.usage)
                    ...
                    if attempt == CLAUDE_MAX_RETRIES - 1:
                        logger.error(f"Brief: API error on {model}: {e}")
                    else:
